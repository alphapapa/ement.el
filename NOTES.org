...
** [[https://github.com/matrix-org/pantalaimon/issues/154#issuecomment-1951591191][Pantalaimon guide by @Arnavion]]
...
I just set it up. Here are the steps for Linux, as of 2024-02-18, using `pantalaimon-0.10.5` installed from pip:
    ...
    3. In case you have prior experiments, clean the slate by deleting the `~/.local/share/pantalaimon` directory.
    ...
    16. If you have other unverified sessions from pantalaimon, you can sign out of them too.
...
+ [X] Commit changes
  ...
  + [X] Changelog
+ [X] Lint/test
+ [X] Make ~Release:~ commit updating version to ~FOO~
  + [X] In README.org
  + [X] in ement.el
+ [X] Merge (non-fast-forward) into ~stable~ branch (for ELPA)
+ [X] Push ~stable~ branch to repo
+ [X] Make ~Meta:~ commit on ~master~ updating version to ~FOO+1-pre~
  + [X] In README.org
  + [X] in ement.el
+ [X] Push ~Meta:~ commit to ~master~
+ [X] Make release on GitHub
+ [X] Announce release on Reddit.
+ [X] Announce release to TWIM room.
...
I've spent too much time asking people to provide basic info.  Maybe this will help.

+ See: [[https://docs.github.com/en/communities/using-templates-to-encourage-useful-issues-and-pull-requests/configuring-issue-templates-for-your-repository][Configuring issue templates for your repository - GitHub Docs]]
  ...
  #+end_src
** TWIM announcement template

#+begin_src org
  TWIM: The following update is pre-formatted as Markdown for your convenience:

  ,#+begin_src markdown
  [Ement.el](https://github.com/alphapapa/ement.el), a Matrix client for the [GNU Emacs](https://www.gnu.org/software/emacs/) text editor and Lisp environment, has been [released](INSERT-RELEASE-LINK) at version [INSERT-VERSION].  Changes since the last release include:
